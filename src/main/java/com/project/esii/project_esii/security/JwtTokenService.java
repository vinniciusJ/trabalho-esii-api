...
public class JwtTokenService {
    ...
    public RecoveryJwtTokenDto generateToken(BaseUser user) {
        try {
            Algorithm algorithm = Algorithm.HMAC256(SECRET_KEY);

            String role = user.getPersonRole().name();

            // Gera o token com informações específicas
            return new RecoveryJwtTokenDto(JWT.create()
                    .withIssuer(ISSUER)
                    .withIssuedAt(creationDate())
                    .withExpiresAt(expirationDate())
                    .withSubject(user.getEmail())
                    .withClaim("role", role)
                    .withClaim("cpf", user.getCpfNumber())
